...
# ---------- Strategy: GrowthZone calendar platforms (e.g. GABA) ----------
# GrowthZone-powered chamber sites (members.<chamber>.org/...-events) list
# each event's weekday/month/day WITHOUT a year, so the generic date
# regex (which requires a 4-digit year) never matches. Every event link
# reliably contains "/Details/" in its URL though, which is what this
# strategy anchors on instead of guessing from page layout.

_MONTH_NAMES = ("January|February|March|April|May|June|July|August|"
                "September|October|November|December")
_GROWTHZONE_DATE = re.compile(rf"\b({_MONTH_NAMES})\s+(\d{{1,2}})\b")
_GROWTHZONE_TIME = re.compile(r"\b\d{1,2}:\d{2}\s*[AP]M\s*-\s*\d{1,2}:\d{2}\s*[AP]M\b", re.IGNORECASE)


def strategy_custom_growthzone(chamber_name: str, events_url: str) -> list[Event]:
    resp = fetch(events_url)
    if not resp:
        return []
    soup = BeautifulSoup(resp.text, "html.parser")
    ...
    for a in soup.find_all("a", href=True):
        if "/Details/" not in a["href"]:
            continue
        title = a.get_text(strip=True)
        if not title:
            continue  # the image link to the same event has no text; skip it
        url = urljoin(events_url, a["href"])
        if url in seen_urls:
            continue
        seen_urls.add(url)

        # Walk up a few parent levels to find a block of text containing
        # the weekday/month/day near this event's link.
        context = ""
        node = a
        for _ in range(4):
            ...
            if _GROWTHZONE_DATE.search(context):
                break

        m = _GROWTHZONE_DATE.search(context)
        if not m:
            continue
        month_name, day = m.group(1), m.group(2)

        # No year is printed — infer it: use this year, unless that date
        # has already passed, in which case it must mean next year.
        today = datetime.now()
        year = today.year
        try:
            candidate = datetime.strptime(f"{month_name} {day} {year}", "%B %d %Y")
        except ValueError:
            continue
        ...
        events.append(Event(
            chamber=chamber_name,
            title=title,
            date_raw=f"{month_name} {day}, {year} {time_str}".strip(),
